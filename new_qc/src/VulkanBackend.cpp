...
// ---------------------------------------------------------------------------
// Physical device + logical device + pools
// ---------------------------------------------------------------------------
    ...
    for (uint32_t i = 0; i < gpuCount; ++i)
    {
        ...
        for (uint32_t j = 0; j < queueCount; ++j)
        {
            if (queues[j].queueFlags & VK_QUEUE_GRAPHICS_BIT)
            {
                ...
                err = vkGetPhysicalDeviceSurfaceSupportKHR(gpus[i], j, surface_, &presentSupport);
...
void VulkanBackend::initialize(GLFWwindow* window)
{
    ...
    if (w <= 0 || h <= 0) glfwGetWindowSize(window, &w, &h);
    if (w <= 0 || h <= 0) { w = 800; h = 600; }

    // Clamp to surface capabilities
    {
        VkSurfaceCapabilitiesKHR caps;
        VkResult capErr = vkGetPhysicalDeviceSurfaceCapabilitiesKHR(physicalDevice_, surface_, &caps);
        if (capErr == VK_SUCCESS && caps.maxImageExtent.width > 0 && caps.maxImageExtent.height > 0)
        {
    ...
    // Guard swapchain creation against driver-level SIGSEGV
    {
        ...
        else
        {
            ...
            throw std::runtime_error(
                ...
                "driver over SSH/X11 forwarding). Try --backend opengl2 as a fallback.");
        }
        s_segvGuardActive = 0;
        sigaction(SIGSEGV, &oldAction, nullptr);
    }
...
void VulkanBackend::rebuildSwapchain(int width, int height)
{
...

    {
        ...
        else
        {
            ...
            sigaction(SIGSEGV, &oldAction, nullptr);
            throw std::runtime_error(
                "Vulkan swapchain rebuild crashed. Try --backend opengl2 as a fallback.");
        }
        s_segvGuardActive = 0;
        sigaction(SIGSEGV, &oldAction, nullptr);
    }
...
std::vector<uint8_t> VulkanBackend::captureScreenshot(int& width, int& height)
{
    ...
    bool swizzleBR = (format == VK_FORMAT_B8G8R8A8_UNORM || format == VK_FORMAT_B8G8R8A8_SRGB);
    ...
    if (err != VK_SUCCESS) { vkDestroyBuffer(device_, stagingBuffer, allocator_); return {}; }
