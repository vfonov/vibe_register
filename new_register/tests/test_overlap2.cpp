/// test_overlap2.cpp — overlay rendering correctness test (sq1 + sq2).
///
/// Usage: test_overlap2 <sq1.mnc> <sq2.mnc> <tests_dir>
///
/// Renders the axial, sagittal, and coronal central slices of a two-volume
/// overlay (sq1 in GrayScale, sq2 in GrayScale) using renderOverlaySlice()
/// and pixel-compares the output against reference PNGs in <tests_dir>:
///   sq2_ovl_ax.png, sq2_ovl_sa.png, sq2_ovl_co.png
///
/// sq1.mnc and sq2.mnc have the same voxel sampling; the test validates
...
int main(int argc, char** argv)
{
    if (argc < 4)
    {
        std::cerr << "Usage: test_overlap2 <sq1.mnc> <sq2.mnc> <tests_dir>\n";
        return 1;
    }
    ...
    // --- Test 1: axial (view 0) ---
    {
        ...
        {
            std::string ref = testsDir + "sq2_ovl_ax.png";
    ...
    // --- Test 2: sagittal (view 1) ---
    {
        ...
        {
            std::string ref = testsDir + "sq2_ovl_sa.png";
            int mm = comparePng(ref, slice.pixels, slice.width, slice.height);
            if (mm < 0)
                FAIL("could not load reference PNG");
            else if (mm > 0)
    ...
    // --- Test 3: coronal (view 2) ---
    {
        ...
        {
            std::string ref = testsDir + "sq2_ovl_co.png";
            int mm = comparePng(ref, slice.pixels, slice.width, slice.height);
            if (mm < 0)
                FAIL("could not load reference PNG");
            else if (mm > 0)
