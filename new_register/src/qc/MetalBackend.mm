// MetalBackend.mm — native macOS Metal renderer for new_qc.
//
...
//
// Mirrors ImGui's example_apple_metal (and new_register's own MetalBackend.mm):
// a CAMetalLayer-backed MTKView, an imgui_impl_metal renderer, and a per-frame
// command buffer + render pass. Window/input is driven natively by
// main_macos.mm via imgui_impl_osx.
...
struct MetalBackend::Impl
{
    ...
    // App-owned textures, keyed by their ImTextureID pointer value.
...
void MetalBackend::notifyResize(int /*width*/, int /*height*/)
{
    ...
    // rebuild so the main loop can react on the next frame.
...
void MetalBackend::endFrame()
{
    ...
    [encoder pushDebugGroup:@"new_qc ImGui"];
    if (drawData != nullptr)
